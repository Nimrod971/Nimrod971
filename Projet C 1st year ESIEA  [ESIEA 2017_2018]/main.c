...
void colors() 
{
	...
	init_color(COLOR_MAGENTA, 300, 200, 150); /*(== BROWN)*/
...
{
	system("pkill play");
	system("play -q ./Music/Menu.mp3 repeat 99 vol -5db&");
}
...
void INIT_RANDOM_MODE(int MaxY, int MaxX, int dev)
{
	...
	{
		system("pkill play");
		system("play -q ./Music/Hard_Desicion.mp3 repeat 99 vol -9db&");
	...
	{
		system("pkill play");
		system("play -q ./Music/devActivated.mp3 vol -15db&");
		clear();
		attron(COLOR_PAIR(3));
	...
	{
		system("pkill play");
		system("play -q ./Music/devDesactivated.mp3 vol -15db&");
		clear();
		attron(COLOR_PAIR(3));
	...
	system("pkill play");
	system("play -q ./Music/StayWithMe.mp3 repeat 99 vol -12db&");
...
//BOX CAT
	mvprintw(MaxY/8+11,MaxX/2-19,"CAT");
		for(j=0; j<MaxX; j++)
		{
...
//BOX WHALE
	mvprintw(MaxY/8+11,MaxX/2+16,"WHALE");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+10 && j==MaxX/2+10)
			{
...
//BOX EIFFEL TOWER
	mvprintw(MaxY/8+17,MaxX/2-23,"EIFFEL TOWER");
		for(j=0; j<MaxX; j++)
		{
...
//BOX CRANE
	mvprintw(MaxY/8+17,MaxX/2+16,"CRANE");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+16 && j==MaxX/2+10)
			{
...
//BOX HEART
	mvprintw(MaxY/8+23,MaxX/2-21,"HEART");
		for(j=0; j<MaxX; j++)
		{
...
//BOX FLOWER
	mvprintw(MaxY/8+23, MaxX/2+15,"FLOWER");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+22 && j==MaxX/2+10)
			{
...
void INIT_CAT(int MaxY, int MaxX, int dev)
{
	...
	system("pkill play");
	system("play -q ./Music/Beautiful.mp3 repeat 99 vol -15db&");
...
void LETTERS_MENU(int MaxY, int MaxX) 
{
    ...
    mvprintw(MaxY/8+1,MaxX/2-5,"PICROSS GAME");
		...
		//BOX KYU
	mvprintw(MaxY/8+11,MaxX/2-19,"KYU");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+10 && j==MaxX/2-26)
			{
		...
		//BOX NIMROD
	mvprintw(MaxY/8+11,MaxX/2+16,"NIMROD");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+10 && j==MaxX/2+10)
			{
...
//BOX ESIEA
	mvprintw(MaxY/8+17,MaxX/2-21,"ESIEA");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+16 && j==MaxX/2-26)
			{
...
//BOX PARIS
	mvprintw(MaxY/8+17,MaxX/2+16,"PARIS");
		for(j=0; j<MaxX; j++)
		{
			if(i==MaxY/8+16 && j==MaxX/2+10)
			{
...
void INIT_KYU(int MaxY, int MaxX, int dev)
{
	...
	system("pkill play");
	system("play -q ./Music/IBelieve.mp3 repeat 99 vol -15db&");
